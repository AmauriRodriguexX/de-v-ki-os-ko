<script lang="ts">
  ...
  const promoCategories = [
    ...
    { label: 'Súper Lunes', slug: 'super-lunes', icon: Star, color: '#FF0200' },
...
<header class="sticky top-0 z-40 w-full" style="background:rgba(255,255,255,0.66); box-shadow:0 4px 30px rgba(0,0,0,0.1); backdrop-filter:blur(20px); -webkit-backdrop-filter:blur(20px)">
  <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-4">
    ...
    <nav class="hidden items-center gap-0.5 lg:flex">
      ...
      <div class="relative" onmouseenter={() => enter('sucursales')} onmouseleave={leave} role="none">
        <a href="/sucursales" class="flex items-center gap-1.5 rounded-lg px-4 py-2 no-underline transition-all duration-200"
        ...
        {#if openMenu === 'sucursales'}
          <div class="absolute left-0 top-full mt-1 rounded-xl py-2" style="background:#fff; box-shadow:0 12px 40px rgba(0,0,0,0.12); border:1px solid #E2E7F2; min-width:260px; animation:menuFadeIn 0.15s ease-out">
            ...
            {#each sucursalesCities as c (c.slug)}
              <a href={`/sucursales/${c.slug}`} class="flex items-center gap-3 px-4 py-2.5 no-underline transition-colors hover:bg-[#EBF6FD]" style="color:#1A1D2E">
      ...
      <div class="relative" onmouseenter={() => enter('promociones')} onmouseleave={leave} role="none">
        <a href="/promociones" class="flex items-center gap-1.5 rounded-lg px-4 py-2 no-underline transition-all duration-200"
        ...
        {#if openMenu === 'promociones'}
          <div class="absolute left-0 top-full mt-1 rounded-xl py-2" style="background:#fff; box-shadow:0 12px 40px rgba(0,0,0,0.12); border:1px solid #E2E7F2; min-width:260px; animation:menuFadeIn 0.15s ease-out">
            ...
            {#each promoCategories as cat (cat.slug)}
              <a href={`/promociones/${cat.slug}`} class="flex items-center gap-3 px-4 py-2.5 no-underline transition-colors hover:bg-[#F7F9FF]" style="color:#1A1D2E">
      ...
      <!-- YAYA -->
      <a href="/yaya" onmouseenter={() => (openMenu = null)} class="flex items-center gap-1.5 rounded-lg px-4 py-2 no-underline transition-all duration-200"
      ...
      <div class="relative" onmouseenter={() => enter('servicios')} onmouseleave={leave} role="none">
        <a href="/servicios" class="flex items-center gap-1.5 rounded-lg px-4 py-2 no-underline transition-all duration-200"
          ...
          <div class="absolute right-0 top-full mt-1 rounded-xl px-4 py-3" style="background:#fff; box-shadow:0 12px 40px rgba(0,0,0,0.12); border:1px solid #E2E7F2; width:380px; animation:menuFadeIn 0.15s ease-out">
            ...
            <div class="grid grid-cols-3 gap-2">
              {#each serviciosItems as svc (svc.slug)}
                <a href={`/servicios/${svc.slug}`} class="flex flex-col items-center gap-1.5 rounded-xl px-2 py-3 no-underline transition-all hover:-translate-y-0.5" style="background:#EBF6FD; color:#045098">
    ...
    <div class="hidden items-center gap-3 lg:flex">
      ...
      <a href="/facturacion" class="flex items-center gap-1.5 px-4 py-2 no-underline transition-colors" style="border:2px solid #1387C7; color:#1387C7; font-weight:600; font-size:14px; border-radius:16px">
        <FileText class="h-4 w-4" /> Facturación
      </a>
    </div>

    <!-- Hamburguesa móvil -->
    <button class="flex h-10 w-10 cursor-pointer items-center justify-center lg:hidden" onclick={() => (mobileOpen = !mobileOpen)} style="background:none; border:none" aria-label="Menú">
  ...
  <!-- Menú móvil -->
  {#if mobileOpen}
    <div class="overflow-y-auto border-t lg:hidden" style="border-color:#E2E7F2; background:#fff; max-height:calc(100vh - 64px)">
      <nav class="flex flex-col gap-1 p-4">
        {#each [{ k: 'sucursales', label: locLabel }, { k: 'promociones', label: 'Promociones' }, { k: 'servicios', label: 'Servicios' }] as m (m.k)}
          <div class="flex flex-col">
            <button onclick={() => (mobileExpanded = mobileExpanded === m.k ? null : (m.k as OpenMenu))} class="flex items-center gap-2 rounded-xl px-4 py-3" style={`font-weight:600; font-size:16px; color:${isActive('/' + m.k) ? '#1387C7' : '#4A5068'}; background:${mobileExpanded === m.k ? '#F7F9FF' : isActive('/' + m.k) ? '#EBF6FD' : 'transparent'}; border:none`}>
              {m.label}
              <ChevronDown class="ml-auto h-4 w-4 transition-transform" style={`color:#8890AA; transform: rotate(${mobileExpanded === m.k ? 180 : 0}deg)`} />
            </button>
            {#if mobileExpanded === m.k}
              <div class="flex flex-col gap-0.5 pb-2 pl-2" style="animation:menuFadeIn 0.15s ease-out">
                {#if m.k === 'sucursales'}
                  {#each sucursalesCities as c (c.slug)}
                    <a href={`/sucursales/${c.slug}`} class="flex items-center gap-2 rounded-lg px-4 py-2.5 no-underline" style="color:#4A5068; font-size:14px; font-weight:500"><MapPin class="h-3.5 w-3.5" style="color:#1387C7" />{c.label}<span style="font-size:12px; color:#8890AA; margin-left:auto">{c.count}</span></a>
                  {/each}
                {:else if m.k === 'promociones'}
                  {#each promoCategories as cat (cat.slug)}
                    <a href={`/promociones/${cat.slug}`} class="flex items-center gap-2 rounded-lg px-4 py-2.5 no-underline" style="color:#4A5068; font-size:14px; font-weight:500"><span style={`color:${cat.color}`}><cat.icon class="h-4 w-4" /></span>{cat.label}</a>
                  {/each}
                {:else}
                  <div class="grid grid-cols-3 gap-2 px-3 py-2">
                    {#each serviciosItems as svc (svc.slug)}
                      <a href={`/servicios/${svc.slug}`} class="flex flex-col items-center gap-1 rounded-xl px-2 py-2.5 no-underline" style="background:#EBF6FD; color:#045098; font-size:11px; font-weight:700; text-align:center"><svc.icon class="h-4 w-4" />{svc.label}</a>
                    {/each}
                  </div>
                {/if}
              </div>
        ...
        <a href="/yaya" class="flex items-center gap-2 rounded-xl px-4 py-3 no-underline" style={`font-weight:600; font-size:16px; color:${isActive('/yaya') ? '#1387C7' : '#4A5068'}; background:${isActive('/yaya') ? '#EBF6FD' : 'transparent'}`}>
          <img src={yayaLogo} alt="YAYA" class="h-5 w-5 rounded object-cover" /> YAYA
        </a>
        <a href="/facturacion" class="mt-2 flex items-center gap-2 px-4 py-3 no-underline" style="border:2px solid #1387C7; color:#1387C7; font-weight:600; font-size:16px; border-radius:16px">
          <FileText class="h-4 w-4" /> Facturación
        </a>
      </nav>
    </div>
