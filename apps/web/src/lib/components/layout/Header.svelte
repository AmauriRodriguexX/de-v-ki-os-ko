...
<header class="sticky top-0 z-40 w-full transition-transform duration-300" style={`background:rgba(255,255,255,0.66); box-shadow:0 4px 30px rgba(0,0,0,0.1); backdrop-filter:blur(20px); -webkit-backdrop-filter:blur(20px); transform: translateY(${showHeader ? '0' : '-100%'})`}>
  <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-4">
    <!-- Logo -->
    <a href="/" class="flex shrink-0 items-center gap-2 py-2 no-underline">
    ...
    <nav class="hidden items-center gap-0.5 lg:flex">
      ...
      <div class="relative" onmouseenter={() => enter('sucursales')} onmouseleave={leave} role="none">
        <a href="/sucursales" class="flex items-center gap-1.5 rounded-lg px-4 py-2.5 no-underline transition-all duration-200"
      ...
      <div class="relative" onmouseenter={() => enter('promociones')} onmouseleave={leave} role="none">
        <a href="/promociones" class="flex items-center gap-1.5 rounded-lg px-4 py-2.5 no-underline transition-all duration-200"
      ...
      <!-- YAYA -->
      <a href="/yaya" onmouseenter={() => (openMenu = null)} class="flex items-center gap-1.5 rounded-lg px-4 py-2.5 no-underline transition-all duration-200"
      ...
      <div class="relative" onmouseenter={() => enter('servicios')} onmouseleave={leave} role="none">
        <a href="/servicios" class="flex items-center gap-1.5 rounded-lg px-4 py-2.5 no-underline transition-all duration-200"
    ...
    <div class="hidden items-center gap-3 lg:flex">
      ...
      <a href="/facturacion" class="flex items-center gap-1.5 px-4 py-2.5 no-underline transition-colors" style="border:2px solid #1387C7; color:#1387C7; font-weight:600; font-size:14px; border-radius:16px">
  ...
  <div class="fixed bottom-0 right-0 top-0 z-[70] flex h-full w-full max-w-sm flex-col border-l border-[#E2E7F2] bg-white shadow-2xl transition-transform duration-300 ease-out lg:hidden" style="animation: slideIn 0.25s cubic-bezier(0.16, 1, 0.3, 1)">
    ...
    <nav class="flex-1 overflow-y-auto p-6 scrollbar-hide">
      <div class="flex flex-col gap-2">
          ...
          <div class="flex flex-col">
            ...
            {#if mobileExpanded === m.k}
              <div class="mt-1 flex flex-col gap-1.5 pb-2 pl-4" style="animation: menuFadeIn 0.2s ease-out">
                {#if m.k === 'sucursales'}
                  {#each sucursalesCities as c (c.slug)}
                    <a href={`/sucursales/${c.slug}`} class="flex items-center gap-2 rounded-lg py-2.5 pr-2 no-underline active:bg-[#F7F9FF]" style="color:#4A5068; font-size:14px; font-weight:500">
                ...
                {:else if m.k === 'promociones'}
                  {#each promoCategories as cat (cat.slug)}
                    <a href={`/promociones/${cat.slug}`} class="flex items-center gap-2 rounded-lg py-2.5 pr-2 no-underline active:bg-[#F7F9FF]" style="color:#4A5068; font-size:14px; font-weight:500">
                ...
                {:else}
                  <div class="grid grid-cols-2 gap-2 py-2">
                    {#each serviciosItems as svc (svc.slug)}
                      <a href={`/servicios/${svc.slug}`} class="flex flex-col items-center gap-1 rounded-xl px-2 py-3 no-underline active:scale-95" style="background:#EBF6FD; color:#045098; font-size:11px; font-weight:700; text-align:center">
        ...
        <a href="/yaya" class="flex items-center gap-2 rounded-xl px-4 py-3.5 no-underline active:bg-[#EBF6FD]" style={`font-weight:600; font-size:16px; color:${isActive('/yaya') ? '#1387C7' : '#4A5068'}; background:${isActive('/yaya') ? '#EBF6FD' : 'transparent'}`}>
          <img src={yayaLogo} alt="YAYA" class="h-5 w-5 rounded object-cover" /> YAYA
        </a>
        <a href="/facturacion" class="mt-4 flex items-center justify-center gap-2 py-3.5 no-underline active:scale-98" style="border:2px solid #1387C7; color:#1387C7; font-weight:600; font-size:16px; border-radius:16px">
          <FileText class="h-4 w-4" /> Facturación
        </a>
      </div>
    </nav>
